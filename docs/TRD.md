...
- **AuditLog** — id, user id (null only for an access request from a not-yet-whitelisted account; its email is in metadata), project id, environment id, component name, action (`create` | `update` | `delete` | `rollback` | `reveal` | `import` | `request` | `reject` | `login` | `logout` | `refresh`), key (nullable for bulk actions), metadata (jsonb, e.g. old/new version ids), timestamp. Append-only — no update/delete endpoint on this table, ever.
